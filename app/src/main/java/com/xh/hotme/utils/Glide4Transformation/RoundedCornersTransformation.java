...
public class RoundedCornersTransformation implements Transformation<Bitmap> {
    ...
    public boolean equals(Object obj) {
        ...
        return obj instanceof RoundedCornersTransformation &&
                ((RoundedCornersTransformation) obj).radius == radius &&
                ((RoundedCornersTransformation) obj).isLeftTop == isLeftTop &&
                ((RoundedCornersTransformation) obj).isRightTop == isRightTop &&
                ((RoundedCornersTransformation) obj).isRightBotoom == isRightBotoom &&
                ((RoundedCornersTransformation) obj).isLeftBottom == isLeftBottom;
    }

    @Override
    public int hashCode() {
        return ID.hashCode() + (int)radius * 100000 + (isLeftTop?1:0) * 10000 + (isRightTop?1:0) * 1000 + (isLeftBottom?1:0) * 100 + (isRightBotoom?1:0) * 10;
    }

    @Override
    public void updateDiskCacheKey(MessageDigest messageDigest) {
        messageDigest.update((ID + radius + isLeftTop + isRightTop + isLeftBottom+ isRightBotoom).getBytes(CHARSET));
    }
}
